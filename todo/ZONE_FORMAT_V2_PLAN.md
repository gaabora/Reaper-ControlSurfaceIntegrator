...
|---|---|---|---|
| `MIDIExact` | MIDI | Input `Press`, Input `Touch`, Feedback `State` | three-byte `On`; `Off` is optional for Press and required for Touch and State; State also accepts optional `Clear` and `ActiveValues` |
...
| `LastTouchedFXParam`, `FocusedFX`, `SelectedTrackFX`, `FXSlot` passed by clear actions | Select one hard-coded clear operation | Keep separate typed clear actions; these strings are commands, not zone roles | [zone_manager.h](../src/controls/zone_manager.h), [manager_actions.h](../src/actions/manager_actions.h) |
...
- [ ] Implement universal Surface Input decoders and Feedback primitives, then generate their C++, TypeScript, and Lua catalog from one schema. Reject runtime registrations without matching metadata.
  ...
  - ✅ Add universal character-addressed MIDI Text feedback and optional MIDIExact State clear messages plus exact active-value lists. Convert MCU time and assignment displays without device-named runtime processors.
...
- ✅ Compile each `#` binding into channel-specific action-context specifications that reference the original binding by index, while each channel-neutral binding produces one specification and the containing typed zone is never cloned.
  ...
  - [ ] Complete deterministic button gestures and modifier behavior before accepting every parsed declaration at runtime.
    - ✅ Connect unqualified two-state button bindings to `DefaultButtonTrigger`, keep continuous inputs unchanged, apply Product or Device timing with binding overrides, and support effective `Momentary`, `Latch`, and `Hybrid` modes for standard modifier declarations.
    - ✅ Replace the transitional per-`ActionContext` timers and shared Widget flags with one deterministic recognizer per active physical Widget context. Capture the selected action contexts on press, preserve them through modifier changes, order Hold and LongHold milestones by effective delay, run repeats from the same timeline, delay or suppress Tap for exclusive DoublePress, cancel state on zone deactivation, and recover a missing release before a new press. The recognizer accepts an explicit timestamp for deterministic tests.
    - ✅ Add standalone CTest targets for button gestures and resolved gesture reachability. Cover event timing, double-press policies and deadline, repeat cancellation and 32-bit clock wrap, modifier dispatches, missing releases, reset phases, and source-linked context-change conflicts. Keep the recognizer independent from REAPER and WDL headers.
    - ✅ Validate context-changing button event combinations before constructing runtime contexts, using resolved physical Widgets, normalized modifier masks, effective timing, and the shared action-metadata trait. Reject unreachable events, repeated context changes, and invalid same-event action order. Report both conflicting source lines.
    - ✅ Load format 2 zones best effort. Keep valid bindings active when another line has a syntax or runtime error, ignore unavailable relations and navigation targets locally, and execute exact duplicate actions once with a warning.
    - ✅ Run the new gesture CTest targets successfully.
    - [ ] Verify cancellation plus captured modifier contexts in REAPER. Pure recognizer tests do not verify Zone or ModifierManager integration.
    - ✅ Complete multi-action and modifier-source validation, then provide equivalent diagnostics in Bun and OSK live editing.
      - ✅ Add non-blocking C++ diagnostic severity and warnings for additive Press, Hold, LongHold, Release, DoublePress, delayed Exclusive Tap, and a modifier action sharing one physical Widget with another action.
      - ✅ Reject exact duplicate actions, `NoAction` mixed with another action, misplaced `DelayMs` and `RepeatIntervalMs`, non-positive repeat values, and `RunCount>1` or repetition on context-changing and modifier actions before runtime mutation.
      - ✅ Parse binding `DelayMs`, `RepeatIntervalMs`, and `RunCount` as complete unquoted integers. Validate timing overrides against the canonical settings-schema ranges instead of silently accepting `atoi` prefixes or converting invalid text to zero.
      - ✅ Keep context-changing and modifier-state action names in shared C++ macro lists and include both traits in the Bun action catalog.
      - ✅ Add the format 2 Zone parser and equivalent gesture diagnostics to Bun, then expose them through OSK live validation.
        - ✅ Add lossless Bun format 2 Zone detection and parsing for filename identity, metadata, selectors, bindings, `IncludedZones`, `ZoneLayers`, lifecycle blocks, navigation references, and initial structural diagnostics. Align complete-set Main and FX User overrides with the C++ per-Zone-ID model.
        - ✅ Pass the C++-owned context-changing and modifier action traits from the generated action catalog into synchronous editor and CLI validation. Diagnose complete integer timing properties, same-event macros, `NoAction`, context ordering, invariant cross-event reachability, and additive gesture combinations without local action-name lists.
        - ✅ Resolve channel-family selectors to physical Surface Widgets during Bun complete-set validation, apply effective Product and Device timing, resolve unqualified two-state bindings through `DefaultButtonTrigger`, and validate standard modifier sources plus the remaining cross-event conflicts.
        - ✅ Validate the current OSK widget draft through the shared resolved C++ gesture validator before Apply Live. Block errors, preserve concise non-blocking warnings, and use the Surface's effective Device settings for default button events and timing.
    - [ ] Keep `PseudoModifier` declarations and selectors rejected until the separate `PSEUDO_MODIFIER_PLAN.md` runtime work is implemented.
...
- [ ] Replace the native Learn FX dialog with OSK FX edit mode, one in-memory live-preview draft, and atomic User FX-zone save through the shared validated model.
  ...
  - ✅ Retain the parsed format 2 Surface document, expand Learn FX channel-family selectors against its declared channel count, and reject missing Widgets, incompatible Parameter or display capabilities, overlapping selections, and profiles without one usable Parameter Widget.
...
- ✅ Every processor used by `CSI/Surfaces/*/Surface.txt` is classified. Each supported processor must preserve every required address, byte, channel, profile, curve, display field, color, ring, meter, SysEx field, and meaningful option. Each unsupported or ambiguous processor must produce a blocking diagnostic at its source line. The importer must never silently omit a processor or one of its parameters.
  ...
  - ✅ Convert removed legacy `FB_FP8DisplayUpper`, `FB_FP8DisplayUpperMiddle`, `FB_FP8DisplayLowerMiddle`, and `FB_FP8DisplayLower` names to the same universal FaderPort scribble profile as rows 1 through 4. Preserve their old SysEx payload exactly without restoring stale processor aliases in C++.
...
- ✅ Convert legacy Surface Widget blocks through the completed universal Input and Feedback catalog. Move recognized device-specific messages, curves, display fields, colors, rings, meters, and SysEx values into the new metadata. Convert WidgetClass, `StepSize`, and `AccelerationValues` to EncoderProfile references. Convert the exact unclassified standard signed-bit range, remove ignored redundant ranges with a notice, and report other ranges as unresolved.
  ...
  - ✅ Convert ordinary `FB_X32IntProcessor` addresses to OSCInt Value feedback. Block its conditional `/-stat/selidx/<n>` address rewrite and the combined numeric/color `FB_X32Processor` with focused source diagnostics instead of guessing.
