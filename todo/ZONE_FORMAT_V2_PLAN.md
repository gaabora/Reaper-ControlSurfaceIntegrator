...
- ✅ Compile each `#` binding into channel-specific action-context specifications that reference the original binding by index, while each channel-neutral binding produces one specification and the containing typed zone is never cloned.
  ...
  - [ ] Complete deterministic button gestures and modifier behavior before accepting every parsed declaration at runtime.
    ...
    - [ ] Run the new gesture CTest targets and verify cancellation plus captured modifier contexts in REAPER. Pure recognizer tests do not verify Zone or ModifierManager integration.
    - [ ] Add additive/delayed gesture warnings, complete multi-action and modifier-source validation, and share context-effect metadata plus equivalent diagnostics with Bun and OSK live editing.
