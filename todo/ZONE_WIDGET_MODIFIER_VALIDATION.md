...
- [ ] Implement the deterministic C++ gesture recognizer, scoped modifier state, configuration precedence, runtime validation, and persistent settings.
  ...
  - ✅ Connect format 2 Press, Tap, Release, Hold, LongHold, DoublePress, binding delay and repeat overrides, and standard Momentary, Latch, and Hybrid modifier modes to one Zone-level recognizer per active physical Widget context. Keep continuous input outside the button recognizer, capture the press contexts, and clear the recognizer on zone deactivation.
