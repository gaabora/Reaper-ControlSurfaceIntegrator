...
- Keep complete Devices serialization in `devices_model.lua`. Preserve Product and Device setting overrides returned by C++ so saving device routing does not remove settings owned by General or Logging. Convert internal Boolean `1` and `0` values to canonical configuration `true` and `false` values during serialization.
- Keep Devices on one page with Pages and Surface assignments visible together. Edit Pages and Surface assignments in focused popup forms. Select the I/O definition inside the Surface assignment form, edit the selected definition in a collapsible section in the same form, and put Add MIDI and Add OSC at the end of that selector. Keep the assignment list limited to status, Surface ID, OSK, Edit, and Remove. Do not expose duplicate or Zone-profile file-management actions on this page. Keep Listeners in a collapsed advanced section below the main lists.
- Always provide one exact `Home` Page. Create it in the draft when it is missing, and do not allow the user to rename or remove it. When the product configuration is missing, require at least one I/O definition and one Surface assignment on Home before Create configuration. General settings must explain that setup starts in Devices and provide direct navigation there. Do not create an empty invalid file.
...
- Show Devices runtime state as one green `●` or red `×` plus a short action for the user. Read separate MIDI input/output open states, Surface existence, Zone readiness, and the retained Zone failure from the Devices response. Show a precise runtime issue when a port is available but did not open or a Zone profile cannot run. Do not repeat resolved input and output status below the MIDI form. Confirm removal of an assigned I/O definition, then remove its assignments and connected Listeners only after confirmation.
- Store the source mode as part of each Surface template, Main profile, and FX profile selection. Show one dropdown for each selection with separate Vendor, User override, and `by <username>` User labels. Do not show a second source dropdown. Let FX inherit Main when no separate FX selection is enabled. In the combined mode, describe User files as complete per-zone replacements.
- Do not show request, error, or runtime status text at the top of Devices. The shared footer owns operation status and must show a save error instead of the generic dirty status. Do not show an expected inactive runtime warning inside a new or edited Surface assignment form.
- Keep the standalone editor action and a labeled `Reset devices` action at the right edge of the Devices toolbar. Launch the editor through C++; when it is absent, open ReaPack search for the separate `<PackagePrefix> Configuration Editor` package. Reset devices with REAPER action `42348`, then refresh runtime status without discarding the current draft. Provide an OSK action for each connected Surface assignment.
...
- Keep Product and Device value sources out of permanent Override controls. Show Product scope as `Global` in the user interface while keeping `Product` as the internal schema and protocol name. Show the current source in the field tooltip. Use `Reset to default` for Global values and `Use Global value` for Device values in the field context menu. Selecting the inherited value removes the explicit override.
...
- Read and write format 2 binding colors as `StateColors=[ #RRGGBB, ... ]`. The widget editor must not convert them back to anonymous legacy color blocks.
