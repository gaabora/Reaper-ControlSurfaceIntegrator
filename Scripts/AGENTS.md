...
- When the product configuration is missing, show an empty Devices draft and require at least one I/O Device, one Page, and one Surface assignment before Create configuration. Keep the Pages list and its Add Page action visible when no Page exists. General settings must explain that setup starts in Devices and provide direct navigation there. Do not create an empty invalid file.
