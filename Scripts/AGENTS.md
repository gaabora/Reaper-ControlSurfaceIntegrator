...
- Show Devices runtime state as one green `●` or red `×` plus a short action for the user. Read separate MIDI input/output open states from the Devices response and show a precise runtime issue when either port is available but did not open. Do not repeat resolved input and output status below the MIDI form. Confirm removal of an assigned I/O definition, then remove its assignments and connected Listeners only after confirmation.
