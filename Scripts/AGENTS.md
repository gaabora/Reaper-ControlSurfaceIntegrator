...
- Store `Vendor`, `VendorAndUser`, or `User` source modes with Main and FX profile selections. Let FX inherit Main when no separate FX selection is enabled. In the combined mode, describe User files as complete per-zone replacements.
