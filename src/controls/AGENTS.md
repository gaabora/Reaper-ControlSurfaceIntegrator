...
- Store each format 2 zone's derived runtime target on the `Zone` object. Use it for track-collection mode, selected-track lifetime, Page-scope navigation, and format 2 `GoZone` routing. Filename checks and legacy special-name routing must stay inside the legacy path.
