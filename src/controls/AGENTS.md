...
- Label incoming debug logs with the message kind and named fields: `MIDI received` with Surface and Bytes, `OSC received` with Surface, Address, and Value, and `Widget input` with Surface, Widget, and Value.
...
- OSK configuration batches must edit and validate a temporary typed format 2 Zone document before replacing active contexts through `LoadFormat2ZoneRuntimeBindings`. Errors block Apply Live; warnings remain non-blocking and are returned as concise `WARN` statuses. File saves edit the same typed Zone document, use a completed temporary file, timestamped backup, and recovery on replacement failure. A `Widget#` binding remains one channel-family mapping unit because exact numbered Widgets do not carry the family channel index; OSK must apply and report a family edit for every channel.
- Preserve a standard Modifier declaration and its `Default`, `Momentary`, `Latch`, or `Hybrid` mode through OSK Query, Apply Live, and Save. Keep inherited `Default` separate from its effective runtime mode so Save does not create an explicit override. Never serialize a Modifier declaration as a normal action binding.
- Preserve `Blink` and `Blink=<milliseconds>` on format 2 Modifier declarations through parsing, runtime construction, OSK Query, Apply Live, and Save.
- Vendor Surface templates and zones are read-only. Select the Surface template source and Main and FX sources from each Surface assignment. A Vendor and User Surface template may have the same ID, so runtime loading must honor the explicit template source. Main and FX use `Vendor`, `VendorAndUser`, or `User`; in the combined mode, a User zone replaces the complete Vendor zone with the same case-insensitive ID. Excluded sources cannot block profile validation. Create the User FX directory only when the selected FX mode includes User. A Vendor Main edit requires confirmation and an atomic Main-only copy. A Vendor FX edit copies only that file to the matching User FX path. Reload zones after activating either User copy.
- Keep OSK zone creation in `zone_file_creator.*`. Write only format 2 files to the active profile's User `Main` or `FX` root, derive metadata from the selected user task, validate the complete Vendor plus User profile before writing, create one complete temporary file before rename, reject case-insensitive zone-ID and FX-match conflicts, and never edit a parent zone as part of creation.
...
- Create format 2 binding contexts through `format2_zone_runtime.*` from the existing typed document and compiled binding specifications. Load each Zone best effort: skip only an invalid binding, modifier, lifecycle action, or unavailable relation and keep all independent valid runtime contexts. A syntax or binding diagnostic must not disable the complete Zone when its format version and file-derived ID are usable.
...
- Keep action context-change and modifier-state traits in `format2_action_metadata.*`. Validate resolved button groups through `format2_gesture_validation.*` before runtime mutation, using each physical Widget, normalized modifier mask, effective timing and DoublePress policy. Report both source lines for unreachable event pairs. Keep warnings non-blocking through explicit diagnostic severity.
- Use `Format2DiagnosticSeverity::Error` as the default. A Warning must not make a parsed document, compiled binding set, or runtime binding set invalid, and runtime logging must preserve its level.
- Report a Zone binding or Modifier declaration that references a missing Surface Widget as a warning, skip only that binding, and continue loading the valid bindings. Report an exact duplicate gesture action as a warning and execute it once. Do not report normal additive Press, Hold, Release, or DoublePress combinations as problems.
...
- Retain the parsed format 2 Surface document on `ControlSurface`. Resolve the active User-over-Vendor `LearnFX.fxzon` against that document before Zone runtime construction, including terminal `#` expansion, required Input or Text capabilities, overlap checks, and at least one usable Parameter Widget.
...
- Keep the first Zone initialization failure on `ZoneManager` until a successful reload. Devices must report Surface existence and Zone readiness separately.
