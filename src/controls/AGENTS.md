...
- Keep action context-change traits in `format2_action_metadata.*`. Validate resolved button groups through `format2_gesture_validation.*` before runtime mutation, using each physical Widget, normalized modifier mask, effective timing and DoublePress policy. Report both source lines for unreachable event pairs.
