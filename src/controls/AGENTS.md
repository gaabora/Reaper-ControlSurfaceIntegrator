...
- OSK configuration batches must be validated through the shared resolved gesture validator before replacing active contexts. Errors block Apply Live; warnings remain non-blocking and are returned as concise `WARN` statuses. File saves use a completed temporary file, timestamped backup, and recovery on replacement failure.
