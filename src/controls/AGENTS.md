...
- Resolve unqualified format 2 button bindings through `DefaultButtonTrigger`. Keep explicit Press, Tap, Release, Hold, LongHold, and DoublePress event identity on `ActionContext`, use Product or Device timing values with binding delay overrides, and apply each standard modifier declaration's effective Momentary, Latch, or Hybrid mode. Keep PseudoModifier runtime work in its separate plan.
