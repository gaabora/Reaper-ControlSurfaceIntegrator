#pragma once
//
//  fb_sce24.h — SCE24 family feedback processors:
//    SCE24TwoStateLED, SCE24OLED, SCE24Text, SCE24Encoder.
//
//  Also contains the LEDRingRangeColor struct and GetColorValues() helper
//  used by SCE24Encoder::Configure().
...
public:
    ...
    virtual void Configure(const vector<unique_ptr<ActionContext>> &contexts) override
    {
        ...
        if (properties.get_prop(PropertyType_Push) && pushcolor)
        {
            ...
            color.ringRangeLow = 120; color.ringRangeMedium = 127; color.ringRangeHigh = 15;
            colors.push_back(color);
        }
        else if (ledringcolor)
        {
            ...
            color.ringColor.r = 0; color.ringColor.g = 0; color.ringColor.b = 0;
            color.ringRangeLow = 7; color.ringRangeMedium = 0; color.ringRangeHigh = 0;
            colors.push_back(color);
        }
        ...
        {
            LEDRingRangeColor color;
            color.ringRangeLow = 127; color.ringRangeMedium = 127; color.ringRangeHigh = 15;
            colors.push_back(color);
        }
