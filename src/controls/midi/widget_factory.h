...
//
//  Replaces the 300-line if-else-if chain in ProcessMidiWidget() with a
//  data-driven dispatch table. Each widget type (e.g. "Press", "FB_TwoState")
//  registers a handler lambda that receives a MidiWidgetContext and performs
//  whatever insertions/side-effects that type requires.
//
//  Usage:
//    1. Call MidiWidgetRegistry::EnsureRegistered() once (e.g. from
//       Midi_ControlSurface constructor) to populate the registry.
...
struct MidiWidgetContext
{
    ...
    // Pre-computed lookup keys for CSIMessageGeneratorsByMessage_.
