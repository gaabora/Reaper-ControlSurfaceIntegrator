...
//
//  All parsing logic that was previously inside
//  ZoneManager::LoadZoneFile(Zone*, const char* filePath, const char* widgetSuffix)
//  now lives here.  ZoneManager::LoadZoneFile becomes a one-line delegation.
//
...
void ZoneFileParser::ParseFile(ZoneManager *zm, Zone *zone,
    ...
    try
    {
        ...
        for (string line; getline(file, line);)
        {
            ...
            else if (tokens.size() > 1)
            {
                ...
                zm->GetWidgetNameAndModifiers(tokens[0].c_str(), widgetName,
                    modifier, isValueInverted, isFeedbackInverted,
                    hasHoldModifier, HasDoublePressPseudoModifier,
                ...
                try {
                    context = zone->AddActionContext(widget, modifier, zone,
                                                    tokens[1].c_str(), memberParams);
                } catch (const std::exception &e) {
