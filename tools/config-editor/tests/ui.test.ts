...
describe("browser UI bindings", () => {
    test("declares every used element and references an existing HTML ID", () => {
        ...
        expect(html).toContain('id="task-load-status"');
        expect(EDITOR_JAVASCRIPT).toContain('translate("tasks.loading")');
        expect(EDITOR_JAVASCRIPT).toContain('translate("tasks.loadFailed"');
        ...
        expect(html).toContain('<section id="legacy-resolve-step" class="legacy-step-panel">');
        expect(EDITOR_JAVASCRIPT).not.toContain("legacyResolveStep");
        expect(html).not.toContain('id="legacy-draft-check"');
        expect(html).toContain('id="legacy-draft-target"');
        expect(html).toContain('id="legacy-check-all"');
        expect(html).toContain('id="legacy-draft-discard-all"');
        expect(html).toContain('<button id="legacy-check-all" disabled hidden>Check all</button><button id="legacy-draft-discard-all" disabled hidden>Discard all</button>');
        expect(html).not.toContain('id="validate"');
        expect(EDITOR_JAVASCRIPT).toContain("window.setTimeout(() => { void validateCurrent()");
        expect(EDITOR_JAVASCRIPT).toContain('api("/api/legacy/drafts/discard-all"');
        expect(html).toContain('id="legacy-conflict-all"');
        expect(html).toContain('<label class="legacy-path-control"><span>Old CSI folder</span>');
        expect(html).not.toContain("Use another old CSI folder");
        expect(EDITOR_JAVASCRIPT).toContain('action !== "rename" || candidate.kind === "zone"');
        expect(EDITOR_JAVASCRIPT).toContain('resolution.action === "rename" ? "label" : "span"');
        expect(EDITOR_CSS).toContain(".legacy-rename-target input");
        expect(EDITOR_JAVASCRIPT).toContain('api("/api/legacy/draft"');
        expect(EDITOR_JAVASCRIPT).toContain('option.label = "[" + translatedCapabilities(candidate.capabilities) + "]"');
        expect(EDITOR_CSS).toContain(".legacy-zone-tree > ul { padding-left: 0; }");
        expect(html).toContain('id="legacy-problems-resizer"');
        expect(EDITOR_JAVASCRIPT).toContain('legacyProblemsResizer.addEventListener("pointerdown"');
