...
- Convert a named legacy Bank only when all known parent contexts match its target metadata. Block ambiguous Bank and LeaveSubZone conversion with a source-linked explanation. Convert GoZone Home to GoHome and omit empty relation blocks while preserving comments. Check action arguments and layer-only actions in both bindings and lifecycle blocks.
