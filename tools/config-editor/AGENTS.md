...
- Unknown data produces a warning and remains serializable. Syntax or unsafe identifiers produce editor errors, but the runtime loads every independent valid Zone binding when the format version and file-derived ID are usable.
- The product `.conf` uses unversioned brace blocks. Format 2 Surface, Zone, and `LearnFX.fxzon` files use `@Meta { Version=2 ... }`. Keep legacy Zone parsing only for source files that do not start with a significant `@Meta` line.
- Treat only `//` as a comment in current Surface, Zone, and Learn FX files. Report a leading single `/` or unsupported `#` line as an error with a quick fix. Accept exact `#WidgetType`, `#DisplayRow`, `#RingStyle`, `#DisplayFont`, and `#SupportsColor` directives only while reading legacy Learn FX source.
...
- For an invalid `EnterZoneLayer` target, offer independent fixes to change the caller to `GoZone` or make the target `Role=Layer` and change its exit binding to `ExitZoneLayer`.
- For an ambiguous converted legacy `LeaveSubZone`, offer `GoHome` as a local fix. When one selected Home Zone has a navigation binding to the target, also offer one multi-file fix that adds the target to `ZoneLayers`, changes its Home navigation to `EnterZoneLayer`, sets the target to `Role=Layer`, and changes its exit bindings to `ExitZoneLayer`. Store every changed file as an import draft.
- Let an unknown Zone action be commented out through a line quick fix. For a converted named `Bank` binding with the wrong inherited context, report the generated binding line and offer a multi-file fix that moves only that binding to the one selected Zone with the required Target and BankTarget.
- Preserve legacy migration diagnostics after a converted import draft is edited, and apply only one quick fix at a time.
- Show a visible loading message while the main task cards are disabled during startup. Replace it with the initialization error when loading fails.
...
- Validate the current configuration file automatically after an editor-input debounce. Keep `Check all` for an explicit complete configuration check, and do not show a separate current-file Check button.
...
- Pass generated action traits into synchronous format 2 Zone validation. Keep gesture diagnostics in the parser-independent validator and do not infer context or modifier effects from action names.
- During complete-set validation, resolve terminal `#` families through the assigned Surface `Channels`, resolve a terminal `[Name]` to the physical Widget of its matching `Modifier` or `PseudoModifier` declaration, resolve unqualified two-state bindings through the assigned Device `DefaultButtonTrigger`, and use effective Product plus Device timing. Treat `Modifier` declarations as modifier gesture sources through the named shared action trait instead of as runtime actions.
- Report a Zone binding or Modifier declaration that references a missing Surface Widget as a non-blocking warning, consistent with the C++ runtime. Report an exact duplicate gesture action as a warning and execute it once. Do not report normal additive Press, Hold, Release, or DoublePress combinations as problems.
...
- Require positive `Channels` in Surface `@Meta`, omit it from product Device blocks and I/O forms, and report when one Device is assigned to Surface templates with different channel counts. A converted legacy Surface channel count must include both contiguous numbered Widget families and the largest channel confirmed from processor metadata.
...
- Keep a visible old CSI reload action on the import page. Re-read surface folders and source files while preserving the selected surface, selected zones, target choices, conflict choices, and recovery import drafts. Keep the prior preview and drafts if a changed source cannot be refreshed safely.
- Keep the legacy import workspace in two columns. Put a folder tree with independent file-open buttons and import-selection checkboxes beside the import draft editor, with its vertically resizable Problems panel below. Do not indent the first tree level. Give every folder a tri-state selection checkbox; selecting it expands the folder and selects its files, and clearing it collapses the folder and clears its files. Keep source selection collapsible and render resolution as a static section in the right column. Never switch sections only because a preview was created. Stack the columns on narrow screens.
- Keep old CSI source selection compact. Show the old CSI folder field directly above the legacy surface selector. Use the FX folder checkbox instead of a duplicate global FX checkbox, update preview automatically after selection changes, and do not show a separate manual Preview action.
- Keep the legacy Import action in a permanently visible footer below the right-column steps. Show one exact blocking reason beside it: missing surface or file selection, unresolved widget mappings, validation errors, or unresolved target conflicts. Keep preview cards compact, show Source and the target path relative to the selected profile, then show import status or target conflict as a left-aligned third row. Make every renamed Zone target visible and editable in its card. Edit the complete selected draft target path beside its source editor, and show only target-file errors below that input. Do not duplicate generated source in a preview disclosure. Highlight every unresolved target conflict with danger styling, show explicit Replace, Rename, and Skip actions, provide bulk Replace and Skip for all conflicts and bulk Rename for Zone conflicts, and make the footer reason navigate to the first conflict.
- Validate selected legacy zone references against other selected zones and active zones already present in the target profile. When a matching legacy zone is selected but invalid, name that state and link to the matching source file. When it is not selected and no active target exists, name that state and link to the matching source file.
- Use the `FXZones` folder checkbox as the single group control that selects or clears all `FXZones/` sources without changing ordinary zones.
- Import legacy `Surface.txt` into `Surfaces/User`, `Zones/**/*.zon` into the matching `Zones/User/<profile>/Main` tree, and `FXZones/**/*.zon` into its `FX` tree. Let the user change the stable target profile ID and each zone path inside that user profile. Reject duplicate selected targets. Ignore backup names that do not end in `.zon`, convert legacy Surface and Zone documents to format 2, and preserve zone subdirectories. Convert leading single-slash and unsupported hash comment lines to `//`. Route exact Learn FX directives to the dedicated `LearnFX.fxzon` migration instead of keeping them in normal Zone files.
- Keep every duplicate legacy Learn FX source editable with a defined `LearnFX.fxzon` target. Offer a quick fix that comments out the complete duplicate source so preview can exclude it. Clear its stale selection before rebuilding the preview when its draft is discarded.
- Require confirmation before discarding one or all import drafts. Keep explicit `Check all` and `Discard all` actions in the import workflow top bar beside Help. Remove stale selected source paths when rebuilding a preview so one obsolete Learn FX selection cannot retain old diagnostics.
...
- Preserve legacy Widget `Alias` values as quoted format 2 Widget properties during Surface conversion.
- Allow `Blink` or `Blink=<milliseconds>` on a format 2 `Modifier` declaration. Preserve legacy modifier blinking during import, and convert a declared modifier used as the final part of a legacy Widget expression to terminal `[Name]` syntax. Do not report the modifier declaration and a binding through its matching terminal `[Name]` source as additive actions.
...
- Convert legacy OSC `AnyPress` to OSCFloat Press with `Match=Any`, `Touch` to OSCFloat Touch with `Match=NonZero`, and `FB_IntProcessor` to OSCInt Value feedback. Preserve each explicit legacy address.
- Convert ordinary legacy `FB_X32IntProcessor` addresses to OSCInt Value feedback. Reject its conditional `/-stat/selidx/<n>` rewrite until the format can declare that behavior. Reject `FB_X32Processor` because one legacy processor combines numeric and palette-color updates on one address; require the user to choose typed feedback instead of guessing.
- Convert legacy X32 faders to one bidirectional decibel ValueProfile used by OSCFloat Value input and feedback with a 30 ms echo guard. Convert X32 rotary input to one decode-only ValueProfile that preserves the legacy `1..63` decrease and `64..127` increase ranges, and move its legacy feedback processor to a nested constant OSCInt Acknowledge block.
...
- Convert legacy MCU time display output to `MIDICharacters` plus explicit mode-light State blocks. Convert the assignment display to explicit State blocks with Clear messages. Keep the character addresses, letter codes, and mode values in Surface metadata.
...
- Convert legacy SCE24 LED button feedback to generic MIDISysEx State feedback. Convert `OffColor` and `OnColor` to one ordered `StateColors` list, default an omitted state to black, remove the ignored alpha byte, and reject malformed colors without changing that binding.
- Convert FaderPort 8 and 16 scribble lines to one generic MIDISysEx Text profile. Keep the device type, zero-based hardware channel, row, text width, blank clear value, alignment, and inversion as explicit metadata, and publish a one-based logical Widget Channel.
- Treat legacy `FB_FP8DisplayUpper`, `FB_FP8DisplayUpperMiddle`, `FB_FP8DisplayLowerMiddle`, and `FB_FP8DisplayLower` as removed names for the four FaderPort 8 scribble rows. Convert them to the same generic profile and payload as `FB_FP8ScribbleLine1..4`; do not restore the old names in the runtime.
- Convert FaderPort 8 and 16 peak meters to one linear `0..127` MeterProfile and their documented channel-pressure or program-change status. Use continuous refresh because the hardware peak meters decay automatically. Do not preserve the invalid legacy `value * 0xA0` calculation. https://pae-web.presonusmusic.com/downloads/products/pdf/FaderPort-16-8_OwnersManual_V4_EN_02012020.pdf
- Convert both Icon display variants and their upper and lower rows to the shared seven-character TextProfile. Preserve manufacturer bytes, display type, display row, seven-byte channel offset, silence handling, and one-based logical Widget Channel as declarative data.
- Convert Asparion RGB, encoder ring, text displays, and left/right meters to universal Color, Ring, Text, and Meter feedback. Infer the required track-color Channel only from a complete positive numeric Widget suffix, convert zero-based processor channels to one-based Widget metadata, and preserve every literal MIDI address, display field, width, style code, and meter limit.
- Convert QCon Pro X master meters to one decibel-step MeterProfile and generic MIDI7 Meter output. Keep the left/right selector as a value-prefix nibble, not Widget Channel metadata.
- Convert FaderPort Classic split fader input and feedback to symmetric generic `MIDISplit` values with `Bits=10` and `Commit=LSB`. Keep only the two message prefixes and do not preserve the legacy input/output range mismatch.
...
- Store each changed legacy import source and target path as one debounced temporary draft, restore it after editor restarts, and keep it after a successful import until the user explicitly discards it. Disable Discard draft when it matches the generated source and default target, and never write an import draft back to the old CSI directory. Recheck the original source hash before every preview and import.
- Resolve missing or incompatible legacy zone widgets through capability-filtered target widget mappings. Convert legacy `Widget|` channel families to format 2 `Widget#` families and map them only to another compatible channel family. Treat `Touch` in `Touch+Widget` as a modifier, not as a required touch-input capability on `Widget`. Apply mappings to preview source before source hashes and transactional saves are calculated.
...
- Validate legacy import against complete destination Zone documents, not a set of existing zone names. Repeat complete-profile validation after conflict decisions and before writing. Report missing or invalid relation and navigation targets as non-blocking warnings because the runtime skips them. Keep mixed legacy/format 2 files, duplicate Home roles, invalid relation roles, and structural cycles as blocking import errors. Partial document checks must not require a Home zone.
- Convert a named legacy Bank only when all known parent contexts match its target metadata. Block ambiguous Bank and LeaveSubZone conversion with links to the named target and parent zones. Suppress a secondary Bank amount error on the same binding because the context diagnostic owns that conversion failure. Convert GoZone Home to GoHome and omit empty relation blocks while preserving comments. Check action arguments and layer-only actions in both bindings and lifecycle blocks.
...
- Keep a Help action available in the top application area. Its local dialog must explain the basic edit and import flow and list the supported CodeMirror shortcuts.
