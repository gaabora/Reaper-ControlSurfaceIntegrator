...
- During complete-set validation, resolve terminal `#` families through the assigned Surface `Channels`, resolve unqualified two-state bindings through the assigned Device `DefaultButtonTrigger`, and use effective Product plus Device timing. Treat `Modifier` declarations as modifier gesture sources through the named shared action trait instead of as runtime actions.
...
- Keep the legacy import workspace in two columns. Put a folder tree with independent file-open buttons and import-selection checkboxes beside the import draft editor, with its Problems panel below. Give every folder a tri-state selection checkbox; selecting it expands the folder and selects its files, and clearing it collapses the folder and clears its files. Put source selection and resolution in two independent compact accordion steps in the right column. Keep at least one step open and allow both steps to remain open. Never switch steps only because a preview was created. Stack the columns on narrow screens.
- Keep old CSI source selection compact. Use the FX folder checkbox instead of a duplicate global FX checkbox, update preview automatically after selection changes, and do not show a separate manual Preview action.
