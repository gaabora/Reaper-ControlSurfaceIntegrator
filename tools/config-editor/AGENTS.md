...
- Report a Zone binding or Modifier declaration that references a missing Surface Widget as a non-blocking warning, consistent with the C++ runtime.
...
- Validate legacy import against complete destination Zone documents, not a set of existing zone names. Repeat complete-profile validation after conflict decisions and before writing. Keep missing or invalid required zones, mixed legacy/format 2 files, duplicate Home roles, and invalid navigation as blocking import errors. Partial document checks must not require a Home zone.
