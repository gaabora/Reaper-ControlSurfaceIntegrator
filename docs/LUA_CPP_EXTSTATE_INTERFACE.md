...
`ConfigApplyLive` and `ConfigSave` accept only an active format 2 zone. Both replace the selected physical Widget's bindings in a temporary parsed Zone document. Apply Live sends only the selected parsed bindings through the normal format 2 runtime loader and does not change unrelated Widget contexts. ConfigSave preserves unrelated source lines and writes the validated result. A terminal `#` binding remains one channel-family mapping unit because exact numbered Widget bindings do not carry its channel index. Editing one resolved family member therefore updates every channel in the family and reports the scope. A mixed exact and family mapping for the selected Widget is rejected for manual resolution. Modifier declarations are not normal bindings and remain configuration-editor-only until the OSK protocol can preserve their declaration type and mode. The file write uses a completed temporary file and a timestamped backup.
