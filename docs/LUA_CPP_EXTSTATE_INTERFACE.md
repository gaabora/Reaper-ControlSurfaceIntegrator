...
`ConfigApplyLive` requires an active format 2 zone. `ConfigSave` uses the active target or the last target cached by `ConfigQuery`. Both replace the selected physical Widget's bindings in a temporary parsed Zone document. Apply Live sends only the selected parsed bindings through the normal format 2 runtime loader and does not change unrelated Widget contexts. ConfigSave preserves unrelated source lines and writes the validated result. A terminal `#` binding remains one channel-family mapping unit because exact numbered Widget bindings do not carry its channel index. Editing one resolved family member therefore updates every channel in the family and reports the scope. A mixed exact and family mapping for the selected Widget is rejected for manual resolution. The file write uses a completed temporary file and a timestamped backup.

The serialized OSK binding suffixes `__OSK_MODIFIER_DEFAULT`, `__OSK_MODIFIER_MOMENTARY`, `__OSK_MODIFIER_LATCH`, and `__OSK_MODIFIER_HYBRID` identify a standard Modifier declaration and its mode. Lua preserves the suffix through Query, Apply Live, Save, clone, and reorder operations. `Default` keeps the declaration linked to the current Device setting and is saved without an explicit `Mode` property. A Modifier declaration accepts one standard modifier name and cannot use normal binding selectors or extra action parameters. An exact Modifier declaration combined with a `Widget#` family for the same physical Widget is rejected for manual editing because the two scopes differ.
...
- Each Surface assignment's saved I/O type, template ID, Main and FX profile IDs, `MainSourceMode` and `FxSourceMode`, start channel, matching I/O and Surface runtime state, `ZoneReady`, `RuntimeIssue`, selected User or Vendor template source, and available User and Vendor Zone sources. `Missing` and `Invalid` resource states remain visible in the assignment.
