...
- `Scripts/surface_io_schema.conf` is the only source for format 2 Surface primitive names, capabilities, protocol encodings, allowed and required properties, property value and relationship rules, nested transport blocks, reusable profiles, and Surface-level block schemas. CMake embeds the same schema for C++; Lua and the Bun editor must read it directly instead of adding local lists.
