# CSI Configuration Format Documentation

## Overview

CSI configuration consists of three main file types:
1. **CSI.ini** - Main plugin configuration
2. **.zon files** - Zone definitions (widget-to-action mappings)
3. **.zon files (Surface Templates)** - Hardware widget definitions

## File Locations

All CSI configuration files are located in the REAPER resource path:
...
```
REAPER/
├── CSI.ini                          # Main configuration
├── CSI/
│   ├── CSI.ini                      # Alternative location
│   ├── Zones/
│   │   └── *.zon                   # Zone definitions
│   ├── FXZones/
│   │   └── *.zon                   # FX-specific zones
│   ├── AutoGeneratedFXZones/       # Auto-generated by Learn mode
│   │   └── *.zon
│   └── Surfaces/
│       └── *.zon                   # Surface templates
└── UserPlugins/
    └── reaper_csurf_integrator.*   # Plugin binary
```

## CSI.ini - Main Configuration File
